...
policy_np = NeuralProcess(2, 1, args.r_dim, args.z_dim, args.h_dim, fixed_sigma=0.1).to(args.device_np)
    ...
    for i in range(10):
        ...
        for y_slice in x2:
            ax_stdv.add_collection3d(
                plt.fill_between(x1, stddev_low[i, :].cpu(), stddev_high[i, :].cpu(), color='lightseagreen', alpha=0.02),
                zs=y_slice, zdir='y')
            i += 1
