...
folder_path = '/media/francesco/Irene/Francesco/Master Thesis/scratch/hopper_np_03/'
...
for s, subfolder_path in enumerate(all_folders[1:]):
    ...
    while start < min_len:
        ...
        avg_rews.append(avg)
        steps.append(end)
    if '200rm' in subfolder_path:
        color = 'r'
    elif '100rm' in subfolder_path:
        color = 'g'
    else: color='b'
    if 'loo:False' not in subfolder_path:
