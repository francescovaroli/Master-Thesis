...
folder_path = '/media/francesco/Irene/Francesco/Master Thesis/scratch/np_hop_che/'
...
for s, subfolder_path in enumerate(all_folders[1:]):
    ...
    while start < min_len:
        ...
        avg_rews.append(avg)
        steps.append(end)
    '''if '24epi' in subfolder_path:
        color = 'r'
        label = '24 episodes per iter'
    elif '12epi' in subfolder_path:
        color = 'g'
        label = '12 episodes per iter'
    else:
        color='b'
        label = 'ER size: 50 ep'''
    if 'ctxt:T' in subfolder_path:
        color = 'r'
        label = 'ER as context'
    elif 'ctxt:F' in subfolder_path:
        color = 'g'
        label = 'last iter epiodes as context'
    else:
        color='b'
