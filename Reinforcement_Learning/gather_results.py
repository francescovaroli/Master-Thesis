...
folder_path = '/media/francesco/Irene/Francesco/Master Thesis/scratch/np_hop_critic_1/'
...
for s, subfolder_path in enumerate(all_folders[1:]):
    ...
    while start < min_len:
        ...
        avg_rews.append(avg)
        steps.append(end)
    '''if '60rm' in subfolder_path:
        color = 'r'
        label = 'ER size: 60 ep'
    elif '90rm' in subfolder_path:
        color = 'g'
        label = 'ER size: 90 ep'
    else:
        color='b'
        label = 'ER size: 120 ep'''
    if 'isctxt:T' in subfolder_path:
        color = 'r'
        label = 'RM as context'
    elif 'isctxt:F' in subfolder_path:
        color = 'g'
        label = 'last iter as context'
    else:
        color='b'
