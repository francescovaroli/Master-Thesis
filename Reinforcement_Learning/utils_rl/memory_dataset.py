...
class MemoryDataset(Dataset):
    ...
    Dataset to store memory
        ...
        to check that the traj are correct plot the position over time, if doesn't make sense could be due to
        multiple workers collecting data and appending trans. ot in order
    """
    def __init__(self, memory_list, max_len=None):
        self.data = []
        self.rewards = []
        ...
        for transition in memory_list:
            ...
            if transition.mask == 0:
                unpadded_data.append([torch.tensor(trajectory_states),
                                  torch.tensor(trajectory_actions), len_traj])
        ...
        for unpad_traj in unpadded_data:
            pad_x = torch.zeros([max_len, state_dim])
            pad_y = torch.zeros([max_len, action_dim])
