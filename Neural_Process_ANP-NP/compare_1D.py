...
for i in range(64):
    ...
    ax_data.plot(x.cpu().numpy(), y.cpu().numpy(), c='b', alpha=0.5)
...
for use_attention in [False, True, True]:
    ...
    ax_epoch.plot(np.linspace(0, epochs-1 ,epochs), np_trainer.epoch_loss_history, c=color, label=mdl)
    ...
    ax_post_avg.set_title(mdl + ' Posterior mean and stdv')
