...
parser.add_argument('--pick-context', default=False, metavar='N',
...
run_id = '/{}_VP_pick{}_NP_mean:{}_A_p:{}_A_v:{}_fixSTD:{}_epV:{}_{}ep_{}kl_{}gamma_'.format(args.env_name, args.pick_context,args.use_mean,
...
def plot_improvements(all_dataset, est_rewards, env, i_iter, args, colors):
    ...
    for e, episode in enumerate(all_dataset):
        ...
        if args.env_name == 'Pendulum-v0':
            states = torch.stack([np.arctan2(states[:, 1], states[:, 0]), states[:, 2]], dim=1)
            for axx in [ax, ax_rew]:
                axx.set_xlim(-np.pi, np.pi)
                axx.set_zlim(env.action_space.low, env.action_space.high)
                bounds_high = env.observation_space.high
                bounds_low = env.observation_space.low
    ...
    cb = fig.colorbar(a)
    cb.set_label('Estimated Advantages')
    ax_rew.set_title('Estimated Advantages')
